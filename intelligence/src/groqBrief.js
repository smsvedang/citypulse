export function fallbackBrief(evidence) {
  ...
  const what =
    evidence.top_anomaly != null
      ...
      : `${evidence.active_events || 0} active events were recorded in this zone.`;
  ...
  return {
    ...
    evidence: [
      `Active events: ${evidence.active_events || 0}`,
      evidence.top_anomaly
        ? `Top anomaly: ${evidence.top_anomaly.type} (${evidence.top_anomaly.score})`
        : 'No anomaly above threshold yet',
    ],
    uncertainty: evidence.uncertainty || 'Correlation does not establish direct causation.',
    source: 'fallback',
  };
}

export async function generateGroqBrief(evidence) {
